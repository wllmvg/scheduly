...
def generar_ics(eventos, fechas_por_codigo):
    ...
    for idx, ev in enumerate(eventos):
        ...
        summary = f"{ev['codigo']}: {ev['asignatura']}" if ev["codigo"] else ev["asignatura"]
        ...
        if ev["aula"]:
            lines.append(_fold_line(f"LOCATION:{ev['aula']}"))
 
        lines.append("BEGIN:VALARM")
        lines.append("ACTION:DISPLAY")
        lines.append(_fold_line(f"DESCRIPTION:{summary}"))
